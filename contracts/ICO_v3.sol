...
contract ICOContract {
    ...
    bool public payInJot;
    /* address public commissionContractAddress = 0x3e134C5dAf56e0e28bd04beD46969Bd516932f02; */
    address public commissionContractAddress;
    JOTOracle public jotOracle;
    Token public tokenJOT;
    //address public oracleAddress = 0x30753e4a8aad7f8597332e813735def5dd395028;
    address public oracleAddress;

    uint public quorum; // quorum needed to determine a dispute
    ...
    constructor(address _tokenAddress, address _projectWallet, uint _sealTimestamp, uint _minimumCap,
                         uint _maximumCap, uint _minimalInvestment, address _operator, uint _quorum, bool _payInJot, address _commissionContractAddress ,address _juryOnlineWallet, address _jotTokenAddress, address _oracleAddress) public {
        operator = _operator;
        ...
        quorum = _quorum;
        payInJot = _payInJot;
        commissionContractAddress = _commissionContractAddress;
        juryOnlineWallet = _juryOnlineWallet;
        tokenJOT = Token(_jotTokenAddress);
        /* tokenJOT = Token(0xaa588d3737b611bafd7bd713445b314bd453a5c8); */
        oracleAddress = _oracleAddress;
        jotOracle = JOTOracle(oracleAddress);
    ...
    function seal() public notSealed only(operator) {
        ...
        CommissionContract commissionContract = CommissionContract(commissionContractAddress);
        commissionContract.addNewICO(commission, payInJot);
    }
    ...
    function addInvestContract(address _investContractAddress) public sealed notStarted only(operator) {
        ...
        uint amountToPay = investContract.amountToPay();
        uint etherAmount = amountToPay*(100-commission)/100;
        CommissionContract commissionContract = CommissionContract(commissionContractAddress);
        commissionContract.addNewInvestContract(_investContractAddress, payInJot, etherAmount);
    }
    ...
    function investContractDeposited() public notStarted {
        address inv_contract_address = msg.sender;
        ...
        uint commissionAmount = investContract.commissionAmount();
        /* uint commissionAmount =   1000000; */
        /* uint commissionAmount = 100000000; */

        uint eth_jot_price = jotOracle.JOTtoETH();
        uint commissionAmountJOT = commissionAmount.mul(eth_jot_price);


        if(payInJot){
            tokenJOT.approve(commissionContractAddress, commissionAmountJOT);
            commissionContract.payCommission(payInJot,inv_contract_address,commissionAmount,commissionAmountJOT,projectWallet);
            /* commissionContract.payCommission(true,inv_contract_address,1000,500); */
        } else {
            commissionContract.payCommission(payInJot,inv_contract_address,commissionAmount,0,projectWallet);
        }
    }
